...
from .samplers import DistributedGroupSampler, DistributedSampler, GroupSampler, DistributedBalancedSampler
...
def build_dataloader(dataset,
    ...
    if not init_data:
        ...
        # my test
        if dist:
            sampler = DistributedBalancedSampler(dataset, num_replicas=world_size, rank=rank, num_samples=init_num_samples, shuffle=False)
            batch_size = num_gpus * samples_per_gpu
            num_workers = num_gpus * workers_per_gpu
        else:
            sampler = DistributedBalancedSampler(dataset, num_replicas=1, rank=0, num_samples=init_num_samples, shuffle=False)
            batch_size = num_gpus * samples_per_gpu
            num_workers = num_gpus * workers_per_gpu
