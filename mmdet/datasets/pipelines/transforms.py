...
from mmdet.utils import LVIS_CATEGORIES
...
# reverse-frequency-copy-paste
@PIPELINES.register_module()
class ReFreqCopyPaste(object):
    def __init__(self, thresh=30, rho=0.9999, sampled_num_classes=1, prob=0.5, max_cache=10, copy_times=1,
                 epochs=30, resize_min=1, resize_max=-1):
        self.thresh = thresh
        self.rho = rho
        self.sampled_num_classes = sampled_num_classes
        self.prob = prob
        self.max_cache = max_cache
        self.copy_times = copy_times # copy numbers per class
        self.epochs = epochs
        self.resize_min = resize_min
        self.resize_max = resize_max
        # self.cache_categories = np.array(
        #     [c['id'] - 1 if c['image_count'] <= self.thresh for c in LVIS_CATEGORIES]
        # )
        self.cache_categories, img_num_list = [], []
        # for c in LVIS_CATEGORIES:
        #     if c['image_count'] <= self.thresh:
        #         self.cache_categories.append(c['id'] - 1)
        #         img_num_list.append(c['image_count'])
        for c in LVIS_CATEGORIES:
            self.cache_categories.append(c['id'] - 1)
            img_num_list.append(c['image_count'])
        self.cache_categories = np.array(self.cache_categories)
        # self.categories = np.arange(len(LVIS_CATEGORIES))
        self.memory_cache = {
            c: {
                "objects": [],
                "gt_bboxes": [],
        ...
        self.memory_cache_max_idx = np.zeros(len(LVIS_CATEGORIES), dtype=int)
        # img_num_list = [c['image_count'] if c['id'] - 1 in self.cache_categories for c in LVIS_CATEGORIES]
        effective_num = 1.0 - np.power(rho, img_num_list)
        per_cls_weights = (1.0 - rho) / np.array(effective_num)
        self.per_cls_weights = per_cls_weights / np.sum(per_cls_weights)
    ...
    def random_resize(self, h, w, annot_h, annot_w):
        real_max = int(min(h / annot_h, w / annot_w))
        resize_max = min(self.resize_max, real_max)
        if resize_max <= self.resize_min:
            return annot_h, annot_w
        resize = self.resize_min + (resize_max - self.resize_min) * random.random()
        return resize * annot_h, resize * annot_w

    def create_copy_annot(self, h, w, annot, annots, c):
        annot = annot.astype(np.int)
        annot_h, annot_w = annot[3] - annot[1], annot[2] - annot[0]
        if annot_h == 0 or annot_w == 0:
            return None
        for epoch in range(self.epochs):
            if self.resize_max > 0:
                resized_annot_h, resized_annot_w = self.random_resize(h, w, annot_h, annot_w)
                left_max, right_min = int(resized_annot_w / 2), int(w - resized_annot_w / 2)
                top_max, bottom_min = int(resized_annot_h / 2), int(h - resized_annot_h / 2)
            else:
                left_max, right_min = int(annot_w / 2), int(w - annot_w / 2)
                top_max, bottom_min = int(annot_h / 2), int(h - annot_h / 2)

            if right_min - left_max <= 0 or bottom_min - top_max <= 0:
                continue

            random_x, random_y = random.randint(left_max, right_min), \
                                 random.randint(top_max, bottom_min)
            if self.resize_max > 0:
                xmin, ymin = random_x - resized_annot_w / 2, random_y - resized_annot_h / 2
                xmax, ymax = xmin + resized_annot_w, ymin + resized_annot_h
            else:
            ...
            if xmin < 0 or xmax > w or ymin < 0 or ymax > h:
                continue
            new_annot = np.array([xmin, ymin, xmax, ymax, c]).astype(np.int)
    ...
    def sampling_classes(self):
        randnum = np.sort(np.random.rand(self.sampled_num_classes))
        prob_point = 0
        randnum_index = 0
        select_classes = []
        for index, prob in enumerate(self.per_cls_weights):
            prob_point += prob
            if randnum[randnum_index] <= prob_point:
                while randnum[randnum_index] <= prob_point:
                    select_classes.append(self.cache_categories[index])
                    randnum_index += 1
    ...
    def add_patch_in_img(self, annot, obj, image):
        if self.resize_max > 0:
            obj = cv2.resize(obj, (annot[2] - annot[0], annot[3] - annot[1]))
        image[annot[1]:annot[3], annot[0]:annot[2], :] = obj
        return image

    def update_memory_bank(self, img, gt_bboxes, gt_labels):
        rare_idxs = np.where(np.isin(gt_labels, self.cache_categories))[0]
        if len(rare_idxs) > 0:
            for idx in rare_idxs:
                c = gt_labels[idx]
                gt_bbox = gt_bboxes[idx].copy().astype(np.int)
                obj = img[gt_bbox[1]:gt_bbox[3], gt_bbox[0]:gt_bbox[2], :].copy()
                if self.memory_cache_max_idx[c] == self.max_cache:
                    self.memory_cache[c]["objects"].pop(0)
                    self.memory_cache[c]["gt_bboxes"].pop(0)
                    self.memory_cache_max_idx[c] = self.max_cache - 1
                self.memory_cache[c]["objects"].append(obj)
                self.memory_cache[c]["gt_bboxes"].append(gt_bbox)
                self.memory_cache_max_idx[c] += 1

    def __call__(self, sample):
        if np.random.rand() > self.prob: return sample

        img, gt_bboxes, gt_labels = sample['img'], sample['gt_bboxes'], sample['gt_labels']
        h, w = sample['img_shape'][0], sample['img_shape'][1]
        assert len(gt_labels.shape) == 1
        assert gt_bboxes.shape[0] == gt_labels.shape[0]
        annots = np.hstack((gt_bboxes, np.expand_dims(gt_labels, axis=1)))

        selected_classes = self.sampling_classes()
        annots = annots.tolist()
        for c in selected_classes:
            num_samp_cache = self.memory_cache_max_idx[c]
            if num_samp_cache > 0:
                num_new_samps = min(self.copy_times, num_samp_cache)
                if self.random_select and num_samp_cache > num_new_samps:
                    ...
                    cache_idxs = np.arange(
                        num_samp_cache - num_new_samps, num_samp_cache
                    )
                for i in cache_idxs:
                    annot, obj = self.memory_cache[c]["gt_bboxes"][i], self.memory_cache[c]["objects"][i]
                    new_annot = self.create_copy_annot(h, w, annot, annots, c)
                    if new_annot is not None:
                        try:
                            img = self.add_patch_in_img(new_annot, obj, img)
                            annots.append(new_annot)
                        except:
                            print("error when img = self.add_patch_in_img(new_annot, annot, img)")
                            continue
        self.update_memory_bank(img, gt_bboxes, gt_labels)
        annots = np.array(annots, dtype=np.float32)
        sample['img'] = img
        sample['gt_bboxes'] = annots[:, :4]
        sample['gt_labels'] = np.int64(annots[:, 4])
