...
def plot_confusion_matrix(confusion_matrix,
    ...
    confusion_matrix = \
        confusion_matrix.astype(np.float32) / per_label_sums.clip(1, np.inf) * 100
...
def main():
    ...
    # import pdb
    # pdb.set_trace()
    test_cls_ids = [94, 305, 387, 506, 634, 713, 960, 1053, 1089, 1140]
    data_classes = []
    for test_cls_id in test_cls_ids:
        data_classes.append(list(dataset.CLASSES)[test_cls_id])
    plot_confusion_matrix(
        confusion_matrix[test_cls_ids, :][:, test_cls_ids],
        tuple(data_classes),
