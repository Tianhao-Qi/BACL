...
from openimage_categories import OPENIMAGE_CATEGORIES
...
for pred in tqdm(predictions, desc='Converting predictions '):
    ...
    cat_name = OPENIMAGE_CATEGORIES[cat-1]['freebase_id']
    ...
    img_pred_map[image_id].append([f"{image_id}", f"{cat_name}", f"{conf:.4f}", f"{xmin:.4f}",f"{xmax:.4f}",f"{ymin:.4f}",f"{ymax:.4f}"])
    
    
# collect into per image strings
converted_predictions = [['ImageID', 'LabelName', 'Score', 'XMin', 'XMax', 'YMin', 'YMax']]
for image_id in image_ids:
    results = img_pred_map[image_id]
    for result in results:
        converted_predictions.append(result)
